	...
	public class TcpHelper
	{
			...
			Thread broadcast = new Thread (() => {
				while (true) {
					if (!isClear) {
                        ...
                        foreach ( var k in clientPool.Keys) {
                            if (k.Connected) {
                                k.Send(msg, msg.Length, SocketFlags.None);
                            }
                        }
		...
		private void Accept (IAsyncResult result)
		{
			...
			try {
				...
				Console.WriteLine (string.Format ("客户端 {0} 已连接", client.RemoteEndPoint));
			} catch (Exception ex) {
		...
		private void Recieve (IAsyncResult result)
		{
			...
			if (client == null || !clientPool.ContainsKey (client) || client.Connected == false) {
				return;
			}
			...
			try {
				...
				Console.WriteLine ("客户端 {0} 已断开连接", clientPool [client].Name);
		...
		{
            Console.WriteLine("服务器正在打包发送到客户端的数据...");
